...
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-4.2.0-38B2AC?logo=tailwindcss&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-Postgres-3ECF8E?logo=supabase&logoColor=white)
![Google Gemini](https://img.shields.io/badge/Google_Gemini-2.5_Flash-8E75B2?logo=googlegemini&logoColor=white)
![ElevenLabs](https://img.shields.io/badge/ElevenLabs-scribe__v1-000000?logo=elevenlabs&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-Deployed-000000?logo=vercel&logoColor=white)
 
Contributors: Deepak Pathak, Yash Chaudhari
 
**TeamHarmony** is an AI-powered team formation and evaluation platform that assesses individual personality stability, evaluates team compatibility through a **grounded mixture-of-agents system**, and provides optimized student group assignments using deterministic graph-based clustering. It combines multi-source personality profiling (resume, audio, survey), LLM-powered multi-agent evaluation anchored to deterministic baselines, and algorithmic optimization to deliver structured, explainable outputs — stability scores, recommendations, strengths, weaknesses, and optimized groupings.
...
- **Grounded Mixture-of-Agents Team Evaluation** — A deterministic signals layer first precomputes pairwise compatibility scores and structured survey signals in-memory (zero AI cost, sub-millisecond). Five specialized AI agents (Role Balance, Skill Overlap, Communication Risk, Deadline Stress, MBTI Compatibility) then evaluate team dynamics in parallel — **each anchored to its deterministic baseline** so scores stay reproducible and resilient to prompt/model drift — before a Meta-Agent synthesizes results using weighted aggregation into a final team score, classification (Stable / Medium / Needs Change), and per-member recommendations.
...
6. **Team Evaluation** — On "Confirm & Evaluate", a deterministic signals layer first precomputes pairwise compatibility scores and structured survey signals in-memory (no AI call). Five specialized agents then run in parallel (5 Gemini calls), **each anchored to its precomputed baseline**, followed by the Meta-Agent synthesizing a final assessment (1 Gemini call). Results include team score, classification, per-agent breakdowns, and per-member recommendations.
...
### 2. Team Compatibility Evaluation (Grounded Mixture-of-Agents AI)
 
Triggered when a user clicks "Confirm & Evaluate Team". A **deterministic grounding layer** precomputes ground-truth signals in-memory, then five specialized agents run **in parallel** — each anchored to its deterministic baseline — followed by a Meta-Agent that synthesizes results.
 
#### Deterministic Grounding Layer (`computeTeamSignals`)
 
Before any LLM call, the pipeline computes a set of deterministic signals from data already stored on each member (`estimated_mbti`, `personality_stability_score`, parsed skills, and structured survey answers). This runs **once**, in-memory, in O(n²) — sub-millisecond, no network, no AI — reusing the pure scoring functions in `lib/teacher/scoringUtils.ts`. Each agent receives a baseline score plus a pre-formatted block of real numbers to reason over instead of inventing them:
 
| Agent | Deterministic Baseline Source | Anchor Band | Grounding Strength |
|-------|-------------------------------|-------------|--------------------|
| **MBTI Compatibility** | Average pairwise MBTI complementarity + team axis (E/I, S/N, T/F, J/P) distribution | ±0.12 | Strong |
| **Skill Overlap** | Average pairwise Jaccard skill-diversity + combined coverage set | ±0.12 | Strong |
| **Communication Risk** | Stability-based pairwise comm score + structured survey signals (speaking comfort, conflict style, decision style) | ±0.15 | Medium |
| **Deadline Stress** | Blend of member stability and self-rated organization + stated deadline behavior | ±0.15 | Medium |
| **Role Balance** | Role / self-described group-role variety index | ±0.20 | Soft (LLM-led) |
 
#### Score Anchoring & Fallback
 
Each agent makes a **single** Gemini call (no extra round-trips) and its returned score is **clamped to `[baseline − band, baseline + band]`**. The deterministic layer owns the numeric backbone; the LLM supplies qualitative nuance (strengths, weaknesses, explanation, per-agent recommendation) and may nudge the score within the band. If Gemini returns malformed JSON, the agent **degrades gracefully to the deterministic baseline** rather than throwing and killing the whole evaluation.
 
**Agent Specializations & Weights:**
...
Each agent outputs: `{ score: 0–1 (anchored), recommendation, strengths[], weaknesses[], explanation }`
...
> **Note:** The same `scoringUtils.ts` functions that power this teacher optimizer are now also reused by the team-evaluation grounding layer (Section 2), giving both systems a single, consistent source of deterministic compatibility scoring.
...
| `/api/profile/analyze` | POST | Build canonical profile (1 Gemini call) + run personality stability pipeline (7 Gemini calls) = 8 total |
...
| `/api/teams/[id]/confirm` | POST | Confirm team + deterministic grounding (0 AI calls) + full evaluation pipeline (6 Gemini calls) |
...
- **Zero-Latency Grounding** — The deterministic signals layer (`computeTeamSignals`) runs once in-memory *before* the parallel agent fan-out (sub-millisecond, no network, no AI), so grounding adds no round-trips; team evaluation keeps its 5-parallel + 1-meta call profile. Determinism is moved to a precompute step rather than a per-agent tool-calling loop precisely to preserve latency.
- **Deterministic Score Anchoring** — Each team agent's score is clamped to a band around its precomputed baseline, making team scores reproducible across runs and resilient to prompt/model drift, while the LLM still contributes qualitative interpretation.
...
│   ├── signals.ts                       # Deterministic grounding layer (precomputed team signals + baselines)
│   ├── grounded.ts                      # Single-call agent wrapper: baseline anchoring + graceful fallback
│   ├── role-balance.ts                  # Team role diversity agent (grounded)
│   ├── skill-overlap.ts                 # Skill complementarity agent (grounded)
│   ├── communication-risk.ts            # Communication conflict agent (grounded)
│   ├── deadline-stress.ts               # Stress resilience agent (grounded)
│   ├── mbti-compatibility.ts            # MBTI synergy agent (grounded)
│   ├── meta-agent.ts                    # Weighted synthesis + classification
│   └── team-evaluation-pipeline.ts      # Grounding + 5-agent grounded parallel pipeline
├── teacher/
│   ├── scoringUtils.ts                  # Pairwise compatibility formulas (shared with grounding layer)
...
- **Deterministic-anchored team scoring** — agent scores are reproducible across runs and largely insulated from prompt/model drift
- **Crash-safe agents** — malformed LLM output degrades to the deterministic baseline instead of failing the evaluation
- **Hybrid symbolic + LLM evaluation** — deterministic scorers own the numbers; the LLM owns interpretation, with a single shared scoring source across teacher optimization and team evaluation
...
- Parallel execution minimizes latency; grounding adds none
- Scalable to 100+ students for group optimization
## Limitations
 
- MBTI inference quality depends on Gemini model consistency (mitigated for team scoring by deterministic anchoring; individual personality estimation is still model-dependent)
- Individual personality stability scoring may still be sensitive to prompt variations across model updates (team scoring is now anchored to deterministic baselines and largely insulated from this)
- **Role Balance** has the weakest deterministic grounding of the five team agents — there is no clean role-coverage formula yet, so its score leans more on LLM judgment within a wider band (±0.20)
...
- **Deterministic role-coverage scorer** — map members to technical/creative/management/analytical and score coverage, giving Role Balance a strong baseline like the other four agents
